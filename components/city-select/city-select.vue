		...
		methods: {
			...
			changeAll(e) {
				if (e.detail.value == 'all') {
					for (let index = 0; index < this.sortItems.length; index++) {
						if (this.sortItems[index].isCity) {
							let cityArr = this.sortItems[index].citys;
							cityArr.forEach((item) => {
								item.checked = true
			...
			checkboxChange(e) {
				let that = this;
				that.selectData = e.detail.value
				/*var values = e.detail.value
				 for (let index = 0; index < that.sortItems.length; index++) {
					if (that.sortItems[index].isCity) {
						let cityArr = that.sortItems[index].citys;
						cityArr.forEach((item) => {
							console.log(values)
							if (item.checked) {
