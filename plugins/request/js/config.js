 ...
 */
export const config = {
    baseURL: "http://183.56.229.65:8090/k3cloud",	
    header: {
        // 'X-Auth-Token': 'xxxx',
...
globalInterceptor.response.use(
    ...
    (err, config) => {
        ...
        showToast(err[0][0]['Result']['ResponseStatus']['Errors'][0]['Message']);
		if(err[0][0]['Result']['ResponseStatus']['Errors'][0]['Message']=="会话信息已丢失，请重新登录"){
			 setTimeout(() => {
			      uni.reLaunch({
